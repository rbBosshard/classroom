  ...
  <section id="kasiski" class="space-y-6 mb-12 scroll-mt-32">
    ...
    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-r">
      <p class="text-gray-700">
        Jahrhundertelang galt Vigenère als unknackbar. Doch im 19. Jahrhundert entwickelten Kryptographen (Charles
        Babbage, Friedrich Kasiski) Methoden, um auch diese Verschlüsselung zu brechen!
      </p>
    </div>
    ...
    <div class="bg-white border-2 border-gray-300 rounded-lg p-4">
      ...
      <p class="text-gray-700 mb-3">
        Der <strong>Kasiski-Test</strong> sucht nach sich wiederholenden Sequenzen im Geheimtext. Wenn dieselbe
        Wortgruppe im Klartext zweimal vorkommt und mit demselben Teil des Schlüssels verschlüsselt wird, entsteht die
        gleiche verschlüsselte Sequenz!
      </p>
      ...
      <div class="mt-4">
        <div class="flex flex-wrap items-center justify-between gap-2 mb-2">
          ...
          <div class="flex gap-2">
            <button @click="loadExampleFromPublicTextFile"
        ...
        <div class="relative">
          <textarea v-model="crackCiphertext"
            @input="crackCiphertext = crackCiphertext.toUpperCase().replace(/[^A-Z]/g, '')"
            placeholder="Gib einen längeren Geheimtext ein oder klicke auf Beispiel laden..." rows="4"
            class="w-full border border-gray-300 rounded-lg px-4 py-3 pr-12 text-sm font-mono focus:ring-2 focus:ring-purple-500 bg-orange-50 text-orange-700 font-bold"></textarea>
          <button v-if="crackCiphertext" @click="copyToClipboard(crackCiphertext)"
        ...
        <div v-if="textLengthHint" class="mt-2 p-3 border rounded text-sm" :class="{
        ...
        <div v-if="crackCiphertext.length > 20 && kasiskiRepeats.length > 0" class="mt-4">
          <div class="flex items-center justify-between mb-2">
            ...
            <div class="flex items-center gap-2">
              <button @click="copyToClipboard(crackCiphertext)"
                class="px-2 py-1 hover:bg-gray-200 rounded transition-colors text-sm" title="Geheimtext kopieren">
                📋
              </button>
              <button @click="previousRepeat" :disabled="currentRepeatIndex === 0"
              ...
              <button @click="nextRepeat" :disabled="currentRepeatIndex >= kasiskiRepeats.length - 1"
                class="px-3 py-1 bg-purple-500 text-white rounded hover:bg-purple-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-sm font-semibold">
                Weiter →
              </button>
            </div>
          ...
          <div class="bg-white border-2 border-purple-300 rounded-lg p-4 font-mono text-sm leading-relaxed">
            <div class="whitespace-pre-wrap break-words">
              <span v-for="(char, index) in crackCiphertext" :key="'char-' + index" :class="getHighlightClass(index)"
                :title="getHighlightTooltip(index)">{{ char }}</span>
            </div>
          </div>
          <div v-if="currentRepeat"
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="text-sm text-gray-700">
                <p><strong>Vorkommen:</strong> {{ currentRepeat.positions.length }}× an Positionen {{
                  currentRepeat.positions.join(', ') }}</p>
                <p><strong>Abstände:</strong> {{ currentRepeat.spacings.join(', ') }}</p>
                <p><strong>Mögliche Schlüssellängen:</strong> <span class="font-mono font-semibold text-green-700">{{
                  currentRepeat.factors.join(', ') }}</span></p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="crackCiphertext.length > 20" class="mt-4">
          ...
          <div v-if="kasiskiRepeats.length > 0" class="space-y-2">
            <div class="max-h-40 overflow-y-auto space-y-1 bg-gray-50 rounded-lg p-2">
              <button v-for="(repeat, index) in kasiskiRepeats" :key="'repeat-' + index"
                @click="currentRepeatIndex = index" :class="[
            ...
            <div class="bg-green-50 border border-green-300 rounded-lg p-3 mt-3">
              ...
              <p class="text-xs text-gray-700">
                ...
                Dies bedeutet, dass sich das Schlüsselwort wahrscheinlich alle {{ estimatedKeyLength }} Buchstaben
                wiederholt.
              </p>
            </div>
          </div>
          <div v-else class="text-gray-500 text-sm">
            Keine signifikanten Wiederholungen gefunden. Versuche einen längeren Text oder einen Text mit mehr
            Wiederholungen.
          </div>
        </div>
      </div>
    ...
    <div class="bg-white border-2 border-gray-300 rounded-lg p-4">
      ...
      <p class="text-gray-700 mb-3">
        Sobald wir die Schlüssellänge kennen, können wir den Text in Gruppen aufteilen. Jede Gruppe wurde mit
      ...
      <div v-if="crackCiphertext.length > 20 && estimatedKeyLength > 0" class="mt-4">
        <div class="mb-3">
          ...
          <div class="flex gap-2 items-center">
            <button @click="analyzeFrequencies"
        ...
        <div v-if="frequencyResults.length > 0" class="space-y-3">
          <div v-for="(result, index) in frequencyResults" :key="'freq-' + index"
          ...
          <div v-if="crackedPlaintext" class="bg-blue-50 border-2 border-blue-400 rounded-lg p-4">
            <div class="flex items-center justify-between mb-2">
              ...
              <button @click="copyToClipboard(crackedPlaintext)"
                class="px-2 py-1 hover:bg-blue-200 rounded transition-colors text-sm" title="Klartext kopieren">
                📋
              </button>
            </div>
            ...
            <div class="bg-white border-2 border-blue-300 rounded-lg p-4 font-mono text-sm leading-relaxed mb-4">
              <div class="whitespace-pre-wrap break-words">
                <span v-for="(char, index) in crackedPlaintext" :key="'plain-' + index"
                  :class="getPlaintextHighlightClass(index)" :title="getPlaintextHighlightTooltip(index)">{{ char
                  }}</span>
              </div>
            </div>

            <p class="text-sm text-gray-600 mt-2">
              💡 Markierte Stellen zeigen, wo im Geheimtext Wiederholungen waren. Sieht das sinnvoll aus? Falls nicht,
              könnte die Schlüssellänge falsch sein oder der Text ist zu kurz für eine genaue Analyse.
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="bg-gradient-to-r from-orange-50 to-red-50 border-l-4 border-orange-500 p-4 rounded-r">
      ...
      <ul class="list-disc list-inside space-y-1 text-gray-700 text-sm">
        <li v-for="(learning, index) in VIGENERE_KASISKI.learnings" :key="'learning-' + index" v-html="learning"></li>
      </ul>
    </div>
  </section>
</template>
