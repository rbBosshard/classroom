<template>
    <section id="quiz" class="space-y-6 mb-12 scroll-mt-32">
        ...
        <div v-for="question in questions" :key="'question-' + question.id"
            ...
            <div class="flex items-start gap-3 mb-4">
                ...
                <div class="flex-1">
                    ...
                    <div class="space-y-2">
                        <label v-for="(option, index) in question.options" :key="'q' + question.id + '-' + index"
                            :class="[
                            ...
                            <input type="radio" :name="'question' + question.id" :value="index"
                                v-model="answers[question.id]" class="mr-2" />
                            <span>{{ option.text }}</span>
                        </label>
                    </div>
                    <div v-if="answers[question.id] !== null && showResults" class="mt-3 p-3 rounded-lg"
                        :class="answers[question.id] === question.correctAnswer ? 'bg-green-50 border border-green-300' : 'bg-red-50 border border-red-300'">
                        <p class="text-sm"
                            :class="answers[question.id] === question.correctAnswer ? 'text-green-800' : 'text-red-800'">
                            {{ answers[question.id] === question.correctAnswer ? '✅ Richtig!' : '❌ Leider falsch.' }}
        ...
        <!-- Results Summary -->
        <div v-if="showResults" class="bg-gradient-to-r from-purple-50 to-blue-50 border-2 rounded-lg p-6">
            ...
            <div class="w-full bg-gray-200 rounded-full h-4 mb-4">
                <div class="h-4 rounded-full transition-all duration-500"
                    :class="score === questions.length ? 'bg-green-500' : score >= 2 ? 'bg-yellow-500' : 'bg-red-500'"
...
function checkAnswers() {
    if (showResults.value) {
        ...
        // Scroll to results
        setTimeout(() => {
            const resultsEl = document.querySelector('.bg-gradient-to-r.from-purple-50.to-blue-50.border-2');
            if (resultsEl) {
