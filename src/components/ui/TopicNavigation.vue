  ...
  <div class="space-y-4">
    ...
    <nav class="bg-white border border-gray-200 rounded-lg p-3 mb-6">
      <div class="flex items-center flex-wrap gap-2 text-sm">
        ...
        <template v-for="(crumb, index) in breadcrumbs" :key="crumb.id">
          ...
          <span v-else class="px-2 sm:px-3 py-1.5 bg-primary-50 text-primary-700 rounded-md font-medium truncate max-w-[150px] sm:max-w-none" :title="crumb.title">
            {{ crumb.title }}
          </span>
        </template>
      </div>
    ...
    <div v-if="currentTopic?.children && currentTopic.children.length > 0">
      <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div 
          v-for="child in currentTopic.children" 
          :key="child.id"
          @click="handleTopicClick(child)"
        ...
        >
          ...
          <h3 class="text-lg font-semibold text-gray-900 mb-2 group-hover:text-primary-600 transition-colors break-words">
            {{ child.title }}
          </h3>
    ...
    <!-- No Content Message -->
    <div v-if="currentTopic && !currentTopic.children?.length && !currentTopic.content" class="crypto-card p-8 text-center">
